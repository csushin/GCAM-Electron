...
function parCoor(filenames, update, country) {
	...
	// console.log(d3.keys(pcData[0]));
	dedupData = [];
	// Normalize slope :(
	if(state.parCoor.plot == "1"){
		dedupData = new Array(pcData.length);
		var labels = d3.keys(pcData[0]),
		labelsRemoved = [];

		// console.log(labels);

		var tagsToRemove = ['name', 'scenario', 'GCAM_ID'];

		for(var i = 0; i < tagsToRemove.length; i++){
			var indexToRemove = labels.indexOf(tagsToRemove[i]);
			if(indexToRemove > -1){
				labelsRemoved.push(labels.splice(indexToRemove, 1));
			}
		}

		var max = new Array(labels.length),
		min = new Array(labels.length);

		// loop and get slope values
		pcData.forEach(function(p, i){
			for(var index = 0; index < labels.length; index++){
				...
				if(!dedupData[i]){
					dedupData[i] = (filenames.length > 2 || typeof(countryIndex) != "undefined") ? {name: p.name, scenario: p.scenario} : {GCAM_ID: p.GCAM_ID};
				}

				dedupData[i][d] = value;

				if(typeof(max[index]) == "undefined"){
					max[index] = value;
					min[index] = value;
				}
				else{
		...
		// console.log('dedupData: ', dedupData);

		var range = new Array(max.length);
		for(var index = 0; index < range.length; index++){
			range[index] = Math.abs(max[index]) >= Math.abs(min[index]) ? Math.abs(max[index]) : Math.abs(min[index]);
		}
		// console.log('range: ', range);
		// return;

		// loop and normalize slope values
		// fill in table data
		if(filenames.length > 2 || typeof(countryIndex) != "undefined")
			tableData = new Array(dedupData.length);

		// TODO: Handle selection > 2
		dedupData.forEach(function(p, i){
			if(!tableData[i])
				tableData[i] = [];

			if(filenames.length > 2 || typeof(countryIndex) != "undefined"){
				// console.log('labelsRemoved: ', labelsRemoved);
				/*for (var index = 0; index < labelsRemoved.length; index++) {
					var d = labelsRemoved[index];

					tableData[i].push(p[d]);
					// console.log(p);
				}*/

				tableData[i].push(p['name']);
				tableData[i].push(p['scenario']);
			}

			for(var index = 0; index < range.length; index++){
				var d = labels[index];

				var scale = d3.scale.linear().domain([-range[index], range[index]]).range([-1,1]);
		...
		if(filenames.length == 1){
			if(state.parCoor.plot == "0"){
				extent = d3.extent(pcData, function(p, i) {
					var value = state.parCoor.year == -1 ? d3.sum(p[d]) + "" : p[d][state.parCoor.year];
					if(!dedupData[i])
